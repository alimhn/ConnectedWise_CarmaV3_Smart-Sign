...
class RoadSegmentList {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.road_segment_list = null;
    }
    else {
      if (initObj.hasOwnProperty('road_segment_list')) {
        this.road_segment_list = initObj.road_segment_list
      }
      else {
        this.road_segment_list = [];
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type RoadSegmentList
    // Serialize message field [road_segment_list]
    // Serialize the length for message field [road_segment_list]
    bufferOffset = _serializer.uint32(obj.road_segment_list.length, buffer, bufferOffset);
    obj.road_segment_list.forEach((val) => {
      bufferOffset = RoadSegment.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type RoadSegmentList
    let len;
    let data = new RoadSegmentList(null);
    // Deserialize message field [road_segment_list]
    // Deserialize array length for message field [road_segment_list]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.road_segment_list = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.road_segment_list[i] = RoadSegment.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.road_segment_list.forEach((val) => {
      length += RoadSegment.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'j2735_msgs/RoadSegmentList';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '2db4607d141b80bd6f38916afcdd0403';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new RoadSegmentList(null);
    if (msg.road_segment_list !== undefined) {
      resolved.road_segment_list = new Array(msg.road_segment_list.length);
      for (let i = 0; i < resolved.road_segment_list.length; ++i) {
        resolved.road_segment_list[i] = RoadSegment.Resolve(msg.road_segment_list[i]);
      }
    }
    else {
      resolved.road_segment_list = []
    }
...
module.exports = RoadSegmentList;
