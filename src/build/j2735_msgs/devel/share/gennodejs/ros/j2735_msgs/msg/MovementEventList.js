...
class MovementEventList {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.movement_event_list = null;
    }
    else {
      if (initObj.hasOwnProperty('movement_event_list')) {
        this.movement_event_list = initObj.movement_event_list
      }
      else {
        this.movement_event_list = [];
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type MovementEventList
    // Serialize message field [movement_event_list]
    // Serialize the length for message field [movement_event_list]
    bufferOffset = _serializer.uint32(obj.movement_event_list.length, buffer, bufferOffset);
    obj.movement_event_list.forEach((val) => {
      bufferOffset = MovementEvent.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type MovementEventList
    let len;
    let data = new MovementEventList(null);
    // Deserialize message field [movement_event_list]
    // Deserialize array length for message field [movement_event_list]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.movement_event_list = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.movement_event_list[i] = MovementEvent.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.movement_event_list.forEach((val) => {
      length += MovementEvent.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new MovementEventList(null);
    if (msg.movement_event_list !== undefined) {
      resolved.movement_event_list = new Array(msg.movement_event_list.length);
      for (let i = 0; i < resolved.movement_event_list.length; ++i) {
        resolved.movement_event_list[i] = MovementEvent.Resolve(msg.movement_event_list[i]);
      }
    }
    else {
      resolved.movement_event_list = []
    }
...
module.exports = MovementEventList;
