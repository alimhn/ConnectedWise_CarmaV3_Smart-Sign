...
class IntersectionStateList {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.intersection_state_list = null;
    }
    else {
      if (initObj.hasOwnProperty('intersection_state_list')) {
        this.intersection_state_list = initObj.intersection_state_list
      }
      else {
        this.intersection_state_list = [];
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type IntersectionStateList
    // Serialize message field [intersection_state_list]
    // Serialize the length for message field [intersection_state_list]
    bufferOffset = _serializer.uint32(obj.intersection_state_list.length, buffer, bufferOffset);
    obj.intersection_state_list.forEach((val) => {
      bufferOffset = IntersectionState.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type IntersectionStateList
    let len;
    let data = new IntersectionStateList(null);
    // Deserialize message field [intersection_state_list]
    // Deserialize array length for message field [intersection_state_list]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.intersection_state_list = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.intersection_state_list[i] = IntersectionState.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.intersection_state_list.forEach((val) => {
      length += IntersectionState.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'j2735_msgs/IntersectionStateList';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '3c3e465a240b59411de29f191c748f3e';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new IntersectionStateList(null);
    if (msg.intersection_state_list !== undefined) {
      resolved.intersection_state_list = new Array(msg.intersection_state_list.length);
      for (let i = 0; i < resolved.intersection_state_list.length; ++i) {
        resolved.intersection_state_list[i] = IntersectionState.Resolve(msg.intersection_state_list[i]);
      }
    }
    else {
      resolved.intersection_state_list = []
    }
...
module.exports = IntersectionStateList;
