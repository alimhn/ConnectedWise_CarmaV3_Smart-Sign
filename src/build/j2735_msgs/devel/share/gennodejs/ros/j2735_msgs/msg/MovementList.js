...
class MovementList {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('movement_list')) {
        this.movement_list = initObj.movement_list
      }
      else {
        this.movement_list = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type MovementList
    // Serialize message field [movement_list]
    // Serialize the length for message field [movement_list]
    bufferOffset = _serializer.uint32(obj.movement_list.length, buffer, bufferOffset);
    obj.movement_list.forEach((val) => {
      bufferOffset = MovementState.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type MovementList
    let len;
    let data = new MovementList(null);
    // Deserialize message field [movement_list]
    // Deserialize array length for message field [movement_list]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.movement_list = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.movement_list[i] = MovementState.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.movement_list.forEach((val) => {
      length += MovementState.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'j2735_msgs/MovementList';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'fc3fb3a1c80422de61bd621e777c7b4d';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new MovementList(null);
    if (msg.movement_list !== undefined) {
      resolved.movement_list = new Array(msg.movement_list.length);
      for (let i = 0; i < resolved.movement_list.length; ++i) {
        resolved.movement_list[i] = MovementState.Resolve(msg.movement_list[i]);
      }
    }
    else {
      resolved.movement_list = []
    }
