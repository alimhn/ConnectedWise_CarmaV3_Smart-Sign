...
class RoadLaneSetList {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.road_lane_set_list = null;
    }
    else {
      if (initObj.hasOwnProperty('road_lane_set_list')) {
        this.road_lane_set_list = initObj.road_lane_set_list
      }
      else {
        this.road_lane_set_list = [];
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type RoadLaneSetList
    // Serialize message field [road_lane_set_list]
    // Serialize the length for message field [road_lane_set_list]
    bufferOffset = _serializer.uint32(obj.road_lane_set_list.length, buffer, bufferOffset);
    obj.road_lane_set_list.forEach((val) => {
      bufferOffset = GenericLane.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type RoadLaneSetList
    let len;
    let data = new RoadLaneSetList(null);
    // Deserialize message field [road_lane_set_list]
    // Deserialize array length for message field [road_lane_set_list]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.road_lane_set_list = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.road_lane_set_list[i] = GenericLane.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.road_lane_set_list.forEach((val) => {
      length += GenericLane.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'j2735_msgs/RoadLaneSetList';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'e7428b9f82ed805425f1b1444faa4165';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new RoadLaneSetList(null);
    if (msg.road_lane_set_list !== undefined) {
      resolved.road_lane_set_list = new Array(msg.road_lane_set_list.length);
      for (let i = 0; i < resolved.road_lane_set_list.length; ++i) {
        resolved.road_lane_set_list[i] = GenericLane.Resolve(msg.road_lane_set_list[i]);
      }
    }
    else {
      resolved.road_lane_set_list = []
    }
...
module.exports = RoadLaneSetList;
