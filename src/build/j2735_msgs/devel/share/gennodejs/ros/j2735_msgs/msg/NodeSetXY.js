...
class NodeSetXY {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.node_set_xy = null;
    }
    else {
      if (initObj.hasOwnProperty('node_set_xy')) {
        this.node_set_xy = initObj.node_set_xy
      }
      else {
        this.node_set_xy = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type NodeSetXY
    // Serialize message field [node_set_xy]
    // Serialize the length for message field [node_set_xy]
    bufferOffset = _serializer.uint32(obj.node_set_xy.length, buffer, bufferOffset);
    obj.node_set_xy.forEach((val) => {
      bufferOffset = NodeXY.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type NodeSetXY
    let len;
    let data = new NodeSetXY(null);
    // Deserialize message field [node_set_xy]
    // Deserialize array length for message field [node_set_xy]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.node_set_xy = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.node_set_xy[i] = NodeXY.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.node_set_xy.forEach((val) => {
      length += NodeXY.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'j2735_msgs/NodeSetXY';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'd666523fe355e90a00861ac4c2085728';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new NodeSetXY(null);
    if (msg.node_set_xy !== undefined) {
      resolved.node_set_xy = new Array(msg.node_set_xy.length);
      for (let i = 0; i < resolved.node_set_xy.length; ++i) {
        resolved.node_set_xy[i] = NodeXY.Resolve(msg.node_set_xy[i]);
      }
    }
    else {
      resolved.node_set_xy = []
    }
